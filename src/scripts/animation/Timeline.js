...
class Timeline {
  ...
  setCurrentFrame(index) {
    ...
    if (this.frames.length > 0 && this.currentFrameIndex < this.frames.length) {
      this.frames[this.currentFrameIndex].pixelData = this.canvas.getPixelData();
    }
  ...
  applyOnionSkin() {
    ...
    const prevIndex = this.currentFrameIndex > 0 ? this.currentFrameIndex - 1 : this.frames.length - 1;
    
    // Get the next frame index
    const nextIndex = this.currentFrameIndex < this.frames.length - 1 ? this.currentFrameIndex + 1 : 0;
