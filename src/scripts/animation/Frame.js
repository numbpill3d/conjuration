...
class Frame {
  ...
  invert() {
    for (let i = 0; i < this.pixelData.length; i++) {
      ...
      // Convert back to hex
      this.pixelData[i] =
        `#${invR.toString(16).padStart(2, "0")}${invG.toString(16).padStart(2, "0")}${invB.toString(16).padStart(2, "0")}`;
    }
  }
  ...
  applyGlitch(intensity = 0.5) {
    ...
    for (let i = 0; i < numRows; i++) {
      ...
      const shiftAmount = Math.floor(
        (Math.random() - 0.5) * this.width * intensity,
      );
  ...
  applyFloydSteinbergDithering() {
    ...
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        ...
        this.pixelData[index] =
          `#${newPixel.r.toString(16).padStart(2, "0")}${newPixel.g.toString(16).padStart(2, "0")}${newPixel.b.toString(16).padStart(2, "0")}`;
