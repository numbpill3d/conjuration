...
function applyChannelShift(ctx, sourceCanvas, width, height, intensity) {
  ...
  const greenOffset = Math.floor((Math.random() - 0.5) * width * intensity * 0.1);
  const blueOffset = Math.floor((Math.random() - 0.5) * width * intensity * 0.1);
...
function applyCRTEffect(ctx, width, height, intensity = 0.5) {
  ...
  if (intensity > 0.3) {
    ...
    ctx.transform(
      1 + intensity * 0.1, 0,
      0, 1 + intensity * 0.1,
      -width * intensity * 0.05, -height * intensity * 0.05
    );
    ...
    ctx.drawImage(
      ctx.canvas,
      0, 0, width, height,
      -width * (scale - 1) / 2, -height * (scale - 1) / 2,
