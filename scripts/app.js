...
class VoidSketchApp {
  ...
  async processGifExport(options) {
    try {
      ...
      const progressToast = this.uiManager.showToast(
        "Preparing frames for GIF export...",
        "info",
        0,
      );
  ...
  async exportSpriteSheet() {
    try {
      ...
      const spriteSheetDataUrl = await this.gifExporter.exportSpriteSheet(
        (progress) => {
          progressToast.updateMessage(
            `Generating sprite sheet... ${Math.round(progress * 100)}%`,
          );
        },
      );
      ...
      if (
        !spriteSheetDataUrl ||
        !spriteSheetDataUrl.startsWith("data:image/png;base64,")
      ) {
