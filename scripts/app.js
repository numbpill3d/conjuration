...
class VoidSketchApp {
  constructor () {
    ...
    this.uiManager.showToast(
      'VOIDSKETCH initialized. Draw in the dark. Animate the echo.',
      'info',
      5000
    )
  ...
  setupEventListeners () {
    ...
    document.addEventListener('ui-toggle-grid', () =>
    ...
    document.addEventListener('ui-export-sprite-sheet', () =>
      this.exportSpriteSheet()
    )

    document.addEventListener('ui-toggle-lore-layer', () =>
      this.toggleLoreLayer()
    )
    ...
    document.addEventListener('ui-save-metadata', (event) =>
      this.saveMetadata(event.detail)
    )
    document.addEventListener('ui-add-sigil', (event) =>
      this.addSigil(event.detail)
    )
    document.addEventListener('ui-apply-glitch', (event) =>
      this.applyGlitch(event.detail)
    )
  ...
  async exportGIF () {
    try {
      ...
      this.uiManager.showToast(
        'Failed to open GIF export options: ' + error.message,
        'error'
      )
    }
  }
  ...
  async processGifExport (options) {
    try {
      ...
      this.uiManager.showToast(
        'Failed to export GIF: ' + error.message,
        'error'
      )
    }
  }
  ...
  async exportSpriteSheet () {
    try {
      ...
      if (result.success) {
        this.uiManager.showToast(
          'Sprite sheet exported successfully',
          'success'
        )
      } else {
        throw new Error(result.error || 'Unknown error')
      ...
      this.uiManager.showToast(
        'Failed to export sprite sheet: ' + error.message,
        'error'
      )
    }
  }
  ...
  toggleLoreLayer () {
    ...
    // Update UI
    this.uiManager.showToast(
      this.state.loreLayer.enabled
        ? 'Lore layer activated'
        : 'Lore layer deactivated',
      'info'
    )
