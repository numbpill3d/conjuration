...
class BrushEngine {
    constructor(pixelCanvas) {
        ...
        this.brushSize = 1;
        this.availableTools = [
            'pencil', 'line', 'rect', 'ellipse', 
            'glitch', 'static', 'eraser', 'fill'
        ];
    ...
    setupEventListeners() {
        ...
        document.getElementById(`brush-${this.currentTool}`).classList.add('active');
    }
...
class SymmetryTools {
    ...
    setupEventListeners() {
        ...
        document.getElementById(`symmetry-${this.currentMode}`).classList.add('active');
    }
    ...
    setMode(mode) {
        ...
        document.querySelectorAll('#tools-panel .tool-button').forEach(element => {
...
class GlitchTool {
    ...
    pixelSort(data, width, height) {
        ...
        for (let y = startY; y < endY; y++) {
            ...
            rowIndices.sort((a, b) => {
                return rowBrightness[rowIndices.indexOf(a)] - rowBrightness[rowIndices.indexOf(b)];
            });
    ...
    addNoise(intensity = null) {
        ...
        const noiseAmount = intensity !== null ? intensity : this.settings.noiseAmount;
