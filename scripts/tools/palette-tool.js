...
class PaletteTool {
  ...
  setupEventListeners () {
    ...
    document
      .getElementById('palette-monochrome')
      .addEventListener('click', () => this.setPalette('monochrome'))
    document
      .getElementById('palette-lain')
      .addEventListener('click', () => this.setPalette('lain'))
    document
      .getElementById('palette-red')
      .addEventListener('click', () => this.setPalette('red'))
    document
      .getElementById('palette-green')
      .addEventListener('click', () => this.setPalette('green'))

    // Mark the default palette as active
    document
      .getElementById(`palette-${this.currentPalette}`)
  ...
  createPaletteSwatches () {
    ...
    paletteSelector.parentNode.insertBefore(
      swatchesContainer,
      paletteSelector.nextSibling
    )
  }
    ...
    if (palette.colors.length === 2) {
      ...
      switch (type) {
        case 'floyd-steinberg':
          ditheringEngine.applyFloydSteinberg(
            palette.colors[1],
            palette.colors[0]
          )
          break
        case 'ordered':
          ditheringEngine.applyOrderedDithering(
            palette.colors[1],
            palette.colors[0]
          )
          break
        case 'noise':
          ditheringEngine.applyNoiseDithering(
            palette.colors[1],
            palette.colors[0]
          )
          break
