...
class PixelCanvas {
  ...
  floodFill (x, y, fillColor) {
    ...
    if (
      this.colorToRGBA(targetColor).join(',') ===
      this.colorToRGBA(fillColor).join(',')
    ) {
      return
    }
    ...
    while (pixelsToCheck.length > 0) {
      ...
      if (
        this.colorToRGBA(currentColor).join(',') !==
        this.colorToRGBA(targetColor).join(',')
      ) {
        continue
      }
      ...
      for (let offsetX = -radius; offsetX <= radius; offsetX++) {
        ...
        if (
          ...
          const sourceX = Math.max(
            0,
            Math.min(this.width - 1, currentX + shiftX)
          )
          const sourceY = Math.max(
            0,
            Math.min(this.height - 1, currentY + shiftY)
          )
