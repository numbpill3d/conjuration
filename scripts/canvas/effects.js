...
class EffectsEngine {
    ...
    updateEffects() {
        ...
        const anyEffectEnabled = Object.values(this.settings).some(effect => effect.enabled);
    ...
    applyGrain() {
        ...
        const imageData = this.effectsCtx.getImageData(0, 0, this.canvas.width, this.canvas.height);
    ...
    applyStatic() {
        ...
        const staticCount = Math.round(this.canvas.width * this.canvas.height * intensity * 0.05);
    ...
    applyCrt() {
        ...
        if (Math.random() < 0.03 * intensity) {
            ...
            const imageData = this.canvas.ctx.getImageData(0, jumpY, width, jumpHeight);
    ...
    setEffectsSettings(settings) {
        ...
        // Update UI controls
        document.getElementById('effect-grain').checked = this.settings.grain.enabled;
        document.getElementById('effect-static').checked = this.settings.static.enabled;
        document.getElementById('effect-glitch').checked = this.settings.glitch.enabled;
        document.getElementById('effect-crt').checked = this.settings.crt.enabled;
        
        // Calculate average intensity for the intensity slider
        const intensities = Object.values(this.settings).map(effect => effect.intensity);
        const avgIntensity = intensities.reduce((sum, val) => sum + val, 0) / intensities.length;
        document.getElementById('effect-intensity').value = Math.round(avgIntensity * 100);
