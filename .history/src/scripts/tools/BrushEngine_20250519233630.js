...
class BrushEngine {
  ...
  applyStaticBrush(x, y, color) {
    ...
    for (let i = -offset; i <= offset; i++) {
      for (let j = -offset; j <= offset; j++) {
        ...
        const probability = 0.7 * (1 - distance / offset);
    ...
    if (this.lastX !== x || this.lastY !== y) {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        // Use a smaller radius for the line to make it more controlled
        ...
        for (let j = -lineOffset; j <= lineOffset; j++) {
          for (let k = -lineOffset; k <= lineOffset; k++) {
            ...
            // Probability decreases with distance from center
            const probability = 0.5 * (1 - distance / lineOffset);
  ...
  applyPatternBrush(x, y, color) {
    // Define some patterns with better variety
    const patterns = [
      // Checkerboard
      [
        [1, 0],
    ...
    // Calculate pattern offset to center it
    const patternOffsetX = Math.floor(patternSize / 2);
    const patternOffsetY = Math.floor(patternSize / 2);
    ...
    for (let i = -offset; i <= offset; i++) {
      for (let j = -offset; j <= offset; j++) {
        ...
        // Use modulo to wrap around the pattern and add patternSize to avoid negative indices
        const patternX = (x + i + patternOffsetX + patternSize) % patternSize;
        const patternY = (y + j + patternOffsetY + patternSize) % patternSize;
